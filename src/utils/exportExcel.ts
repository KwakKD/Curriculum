...
export function excelStatistics_1(): Record<string, number> {
    ...
    const { 학교지정: data, Group: group } = useSchoolJsonDataStore.getState().user[year];
...
export function excelStatistics_3(): Record<string, number> {
    const cache: Record<string, number> = {};
    const { 선택과목: data, Group: group } = useSchoolJsonDataStore.getState().user[year];
    if (!data || !group) return cache
    for (let grade = 1; grade <= 3; grade++) {
        for (let sem = 1; sem <= 2; sem++) {
            let choiceCredit = 0;
...
function excelSubjectNumber(): Record<string, string | number> {
    ...
    for (let grade = 1; grade <= 3; grade++) {
        for (let sem = 1; sem <= 3; sem++) {
            ...
            Object.keys(group).forEach(key => {
                if (group[key].Zone === '학교지정' && group[key].Grade === grade) {
                ...
                } else if (group[key].Zone === '선택과목' && group[key].Grade === grade && group[key].Semester === sem) {
                    const queryData = group[key].Subject.filter(tag => !noGradeSubject.includes(tag)).length
                    if (queryData === 0) {
...
function excelSubjectCredit(): Record<string, number> {
    const cache: Record<string, number> = {}
    const { 학교지정: data } = useSchoolJsonDataStore.getState().user[year];
...
export async function exprotToExcel() {
    ...
    const table1_kor = [...new Set(table1
        .filter(item => item.SubjectGroup === '국어' && item.Tag <= SUBJECT.length)
        .map(item => item.Tag))]
    const table1_math = [...new Set(table1
        .filter(item => item.SubjectGroup === '수학' && item.Tag <= SUBJECT.length)
        .map(item => item.Tag))]
    const table1_english = [...new Set(table1
        .filter(item => item.SubjectGroup === '영어' && item.Tag <= SUBJECT.length)
        .map(item => item.Tag))]
    const table1_history = table1.filter(item => item.SubjectName === '한국사1' || item.SubjectName === '한국사2')
    const table1_society = [...new Set(table1
        .filter(item => item.SubjectGroup === '사회' && item.Tag <= SUBJECT.length && item.SubjectName !== '한국사1' && item.SubjectName !== '한국사2')
        .map(item => item.Tag))]
    const table1_science = [...new Set(table1
        .filter(item => item.SubjectGroup === '과학' && item.Tag <= SUBJECT.length)
        .map(item => item.Tag))]
    const table1_phy = [...new Set(table1
        .filter(item => item.SubjectGroup === '체육' && item.Tag <= SUBJECT.length)
        .map(item => item.Tag))]
    const table1_art = [...new Set(table1
        .filter(item => item.SubjectGroup === '예술' && item.Tag <= SUBJECT.length)
        .map(item => item.Tag))]
    ...
    if (table1_society.length !== 0) {
        if (table1_society.length === 1) {
            ...
            societyCell.value = table1_credit['사회']
