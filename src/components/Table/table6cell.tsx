...
const Table6Cell = React.memo(() => {
    const year = useSchoolJsonDataStore().year;
    const table6Data = useSchoolJsonDataStore((s) => s.user[year].공동교육과정);
    const unionSubject = [...SUBJECT, ...useSchoolJsonDataStore.getState().user[year].AddSubject]
    const inputTable6 = useSchoolJsonDataStore().inputTable6
    const addTable6 = useSchoolJsonDataStore().addTable6
    const handleInputChange = useCallback(
        (e: ChangeEvent<HTMLInputElement>, sub: JsonData, grade: number, sem: number) => {
            ...
            inputTable6(year, inputSubject.Tag, { Credit: num, Grade: grade, Semester: sem });
        },
        [unionSubject, inputTable6]
    )
    ...
    const handleDelete = useCallback(
        (item: JsonData) => {
            addTable6(year, set_sort(table6Data.filter(sub => sub.Tag !== item.Tag)))
